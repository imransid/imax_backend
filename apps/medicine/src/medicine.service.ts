...
import { MedicineData } from "./medic.entity"; // Ensure this entity is in the correct file

@Injectable()
export class MedicineService {
  constructor(
    private readonly prisma: PrismaService // Inject the Prisma service
  ) {}

  async findById(id: number): Promise<MedicineData> {
    try {
      // Find the medicine data by id using Prisma client
      ...
      throw new BadRequestException("Error fetching medicine data");
    }
  }

  async customSeed(): Promise<MedicineData[]> {
    const medicine_Data: MedicineData[] = [
      {
        id: 1,
...

      {
        ...
        indication:
          "Corasis-D tablet is indicated for the treatment & prevention of- Osteoporosis, Osteomalacia, Tetany, Hypoparathyroidism, Disorders of osteogenesis. Corasis-D tablet also used as supplement in case of inadequate intake of calcium in childhood diet, rickets, pregnancy & lactation, elderly patients. Other indications include pancreatitis, phosphate binder in chronic renal failure etc.",
      ...
      },
      {
        ...
        indication:
          "Gaviplus is indicated in-. Gastric reflux. Heartburn. Indigestion. Flatulence associated with gastric reflux. Heartburn of pregnancy. All cases of epigastric and retrosternal distress where the underlying cause is gastric reflux.",
...

      {
        ...
        indication:
          "Nesotem tablet is indicated for the relief of signs & symptoms of-Osteoarthritis. Rheumatoid arthritis. Ankylosing spondylitis & To decrease the risk of developing gastric ulcers in patients at risk of developing NSAID-associated gastric ulcers.",
      ...
      },
      {
        ...
        indication:
          "Trec is used for the treatment of oropharyngeal candidiasis, vulvovaginal candidiasis, pityriasis versicolor, tinea pedis, tinea cruris, tinea corporis, tinea manuum, onychomycosis, histoplasmosis. It is indicated in the treatment of systemic candidiasis, aspergillosis, and cryptococcosis (including ... Read more",
      ...
      // Delete existing records that match the same `id` or `brandName`
    ...
    // return await this.prisma.medicineData.createMany({
    //   data: medicine_Data, // Explicitly typed
    // });

    // console.log("Seeding completed!");

    // let response = await this.prisma.medicineData.findMany();
